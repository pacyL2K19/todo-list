...
import {
  deleteProject, setActiveProject, deleteTask, updateTaskStatus,
} from './Logic';
...
function addTasksListeners() {
  ...
  editTaskBtn.forEach((project) => {
    project.addEventListener('click', function () {
      const index = this.parentNode.getAttribute('data-index'); // grab the index of the clicked task
      editTaskDom.show(index);
    });
  });

  completeTaskBtn.forEach((project) => {
    project.addEventListener('click', function () {
      const index = this.parentNode.getAttribute('data-index');
